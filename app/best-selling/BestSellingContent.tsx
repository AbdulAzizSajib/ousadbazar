...
export default function BestSellingContent() {
  ...
  const [selectedCategoryId, setSelectedCategoryId] = useState<number | null>(() => {
    const id = Number(searchParams.get('category_id'));
  ...
  // Sync category_id from URL when searchParams changes (e.g. navigating from header)
  useEffect(() => {
    const id = Number(searchParams.get('category_id'));
    setSelectedCategoryId(Number.isFinite(id) && id > 0 ? id : null);
  }, [searchParams]);
  ...
  return (
    <section className="w-full px-3 md:px-0 py-3 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[290px_1fr] xl:grid-cols-[350px_1fr] gap-4 items-start">
        {/* Desktop Sidebar */}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="bg-white border border-gray-200 rounded-xl px-3 py-2.5 mb-3 flex justify-between items-center gap-3 md:hidden">
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-1.5 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:border-[#012068] hover:text-[#012068] transition-colors relative"
            >
            ...
            <div ref={infiniteScrollTrigger} className="h-20 flex justify-center items-center">
              {isFetchingNextPage && (
                <div className="flex items-center">
                  <Icon icon="mingcute:loading-line" className="h-10 w-10 animate-spin text-[#012068]" />
